...
import { Palmtree, Gamepad2 } from 'lucide-react';
          ...
          <div className="container mx-auto px-4 flex flex-col md:flex-row justify-between items-center text-xs text-gray-500 space-y-4 md:space-y-0">
            <div className="flex flex-col md:flex-row items-center gap-2 md:gap-4">
                ...
                <span className="text-pink-400/80">
                  Made by <a href="https://guns.lol/soytimes" target="_blank" rel="noopener noreferrer" className="hover:text-pink-300 underline decoration-pink-500/30 underline-offset-4 transition-all">@Soytimes</a>
                </span>
            </div>
            ...
            <div className="flex items-center space-x-6">
                <button 
                  ...
                  className={`hover:text-pink-400 transition-colors ${view === 'home' ? 'text-pink-400' : ''}`}
                >
                  Home
                </button>
                <a href="#" className="hover:text-pink-400 transition-colors cursor-not-allowed opacity-50">Privacy</a>
                <button 
                  onClick={() => setView('terms')} 
                  className={`hover:text-pink-400 transition-colors ${view === 'terms' ? 'text-pink-400' : ''}`}
                >
                  Terms
                </button>
            </div>
